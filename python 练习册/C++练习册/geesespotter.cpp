...
void printBoard(char *board, std::size_t xdim, std::size_t ydim){
    for (int i =0; i < xdim*ydim;i++){
        ...
        if (((int)board[i])/16 == ((int)hiddenBit())/16){
            std::cout<<"*";
        }
        if (((int)board[i])/16 == 0){
            std::cout<<((int)board[i])%16;
        }
        if ((board[i])/16*16 == (int)markedBit()){
            std::cout<<"M";
        }
        std::cout<<" ";
        if (i % ydim == ydim-1){
...
int reveal(char *board, std::size_t xdim, std::size_t ydim, std::size_t xloc,std::size_t yloc){
    ...
    if (xloc + 1 < xdim){
        plane[yloc][xloc+1] = plane[yloc][xloc+1]%16;
    }
    if (xloc >= 1){
        plane[yloc][xloc-1] = plane[yloc][xloc-1]%16;
    }
    ...
    if (yloc+1<ydim){
        ...
        if (xloc+1 < xdim){
            plane[yloc+1][xloc+1] = plane[yloc+1][xloc+1]%16;
        }
        ...
        plane[yloc+1][xloc] = plane[yloc+1][xloc]%16;
    }
    ...
    return plane[yloc][xloc]%16;
}
int mark(char *board, std::size_t xdim, std::size_t ydim, std::size_t xloc,std::size_t yloc){
    if (board[xloc + yloc*ydim] / 16 == 0){
        return 2;
    }else{
        if (board[xloc+yloc*ydim]/16 == markedBit()){
            board[xloc + yloc*ydim] = board[xloc + yloc*ydim]%16 + hiddenBit();
        }else{
            board[xloc + yloc*ydim] = board[xloc + yloc*ydim]%16 + markedBit();
        }
    }
